...
L_code_ptr_bin_apply:
        ...
        mov r15, qword[rsp + 8*4]       ;List of arguments
	mov rbx, r15            ;rbx = list = r15
	mov rcx, 0              ;length counter
.L_loop:
	cmp r15, sob_nil        ;should stop at sob_nil
	je .L_loop_end
	assert_pair(r15)
	mov r15, SOB_PAIR_CDR(r15)      ;next pair
	inc rcx                 ;rcx++
	jmp .L_loop
.L_loop_end:
      	mov rdx, qword [rsp]	;ret
      	mov r8, qword [rsp + 8 * 3]	;f
      	mov r9, qword [rsp + 8 * 4] 	;s
      	lea rax, [8 * rcx - 8* 2]
      	sub rsp, rax
      	mov rdi, rsp
      	mov qword [rdi], rdx
