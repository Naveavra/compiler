...
module Reader : READER  = struct
  ...
  and nt_paired_comment str =
    ...
    let nt3 = star (disj_list
                [pack (word "#\\{") (fun _ -> ());
                 pack (word "#\\}") (fun _ -> ());
                 pack nt_string (fun _ -> ());
                 nt2;
                 pack nt_whitespace (fun _ -> ());
                 nt_comment]) in
    let nt1 = caten nt1 (caten nt3 (char '}')) in
  ...
  and nt_hex_digit str =
      ...
      let nt2 = pack nt2 (let del = int_of_char 'A' - 10 in
                          fun digit -> int_of_char digit - del) in
      let nt1 = disj nt1 nt2 in
          nt1 str
  and nt_nat str =
    ...
    nt1 str
  and nt_hex_nat str = 
    let nt1 = disj nt_hex_digit nt_digit in
    let nt1 = plus nt1 in
    let nt1 = pack nt1
                (fun digits ->
                  List.fold_left
  ...
  and nt_floatA str =
    ...
    let nt1=  pack nt1 (fun (ip, (_, (mant, mult)))->
                  (ip +. mant) *. mult) in
    nt1 str
  ...
  and nt_floatB str =
    let nt1 = caten  (char '.')
                       ( caten nt_mantissa (make_maybe nt_exponent 1.0) ) in
    let nt1 = pack nt1 (fun (_, (mant, exp))->
                  (mant) *.exp
                ) in
    nt1 str
  ...
  and nt_float str =
    ...
    let nt2 = disj_list [nt_floatA;
                         nt_floatB;
                         nt_floatC
                ] in
    let nt1 = pack (caten nt1 nt2) (fun (is_positive, n) ->
                  if is_positive then n else (-.n)) in
    nt1 str
  ...
  and nt_string_part_hex str =
    ...
    nt1 str
  and nt_string_part_dynamic str = (*  (format "<string> ~a " argument-sexpr)  *)
    let nt1 = caten (char '~') (char '{') in
    let nt1 = caten nt1 nt_sexpr in
    let nt1 = caten nt1 (char '}') in
    let nt1 = pack nt1 (fun ((_,sexp),_) -> ScmPair( ScmSymbol "format", ScmPair(ScmString "~a", ScmPair(sexp,ScmNil)))) in
  ...
  and nt_vector str =
    ...
    let nt3 = plus (make_skipped_star nt_sexpr) in
    ...
    let nt1 = pack nt1 (fun (_, scmvec) -> scmvec) in
    nt1 str
         
  (*and nt_proper_list str =
    let nt1 = char '(' in
    let nt2 = caten (star (make_skipped_star nt_sexpr) ) ( char ')') in
    let nt1 = caten nt1 nt2 in
    let nt1 = pack nt1 (fun (_, (sexprs, _)) ->
                List.fold_right
                  (fun curr acc -> ScmPair (curr, acc))
                   sexprs
                   ScmNil   
                ) in
    nt1 str
  ...
  and nt_improper_list str =
    let nt1 = char '(' in
    let nt2 = caten (plus (make_skipped_star nt_sexpr)) (char '.') in (* nt2 = pair(list(sexpr), '.' )*)
    let nt3 = caten nt2 (caten (make_skipped_star nt_sexpr) (char ')')) in (* n3 = pair(nt2, pair(sexpr, ')') ) *)
    let nt1 = caten nt1 nt3 in
    let nt1 = pack nt1 (fun (_, ( (sexpr, _), (lsexp, _) ) ) ->
                  List.fold_right
                    (fun curr acc -> ScmPair (curr, acc))
                    sexpr
                    lsexp
