...
module Tag_Parser (* : TAG_PARSER *) = struct
  ...
  let rec names ribs =
    ...
    |ScmPair(ScmPair(arg, _), rest) -> ScmPair(arg, (names rest))
    | _ -> raise (X_syntax "Unknown form in names");;
  ...
  let rec setbang_exprs exprs = function
    |ScmNil-> exprs
    |ScmPair (rib, ribs) -> ScmPair(ScmPair(ScmSymbol("set!"), rib), (setbang_exprs exprs ribs))
    | _ -> raise (X_syntax "malformed setbang_ribs");;

   let rec what_ever_ribs  = function
    |ScmNil-> ScmNil
    |ScmPair(ScmPair(x,_), ribs) -> ScmPair (ScmPair (x, ScmPair (ScmSymbol "'whatever", ScmNil)), what_ever_ribs ribs)
    | _ -> raise (X_syntax "malformed 'whatever");;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmSymbol var ->
       ...
       then raise (X_syntax "Variabgle cannot be a reserved word")
    ...
    |  ScmPair (ScmSymbol "let",
                ScmPair (ribs, (ScmPair (_, _) as exprs))) ->
       tag_parse (macro_expand_let ribs exprs)

    | ScmPair (ScmSymbol "let*",
               ScmPair (ScmNil, (ScmPair (_, _) as exprs))) ->
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
      tag_parse (ScmPair(ScmSymbol "let", ScmPair(( what_ever_ribs ribs), setbang_exprs exprs ribs))) (*maybe we should change the first ribs to whatever*)
    ...
    | ScmPair (ScmSymbol "or", exprs) ->  
      ...
      | _ -> raise (X_syntax "malforamed or"))
...
module Semantic_Analysis (*: SEMANTIC_ANALYSIS*) = struct
  ...
  let extarct_var' = function
    |ScmVarGet'(v) -> v
    |_ -> raise (X_syntax "malformed");;
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmVarSet(Var v, expr) -> ScmVarSet'(extarct_var'(run (ScmVarGet(Var v)) params env), (run expr params env))
      (* this code does not [yet?] support nested define-expressions *)
      | ScmVarDef(Var v, expr) -> ScmVarDef'(extarct_var'(run (ScmVarGet(Var v)) params env), (run expr params env))
  ...
  let should_box_var name expr params = 
    let (readsLst, writesLst) = find_reads_and_writes name expr params in
    let pairsReadsWrites = cross_product readsLst writesLst in
    let rec run = function
      ...
      | ((Var' (v1, Bound _), env1), (Var' (v2, Bound _), env2)) :: rest -> 
         (not ((find_common_rib name env1) == (find_common_rib name env2)))
         || (run rest)
      | _ :: rest -> run rest
   and find_common_rib name = function
      | [] -> raise (X_this_should_not_happen "var is not in env")
      | rib :: env ->
         if (List.mem name rib) then (rib : string list) else find_common_rib name env in run pairsReadsWrites;;
